...
function daysUntil(dateStr: string) {
    return Math.ceil(
        (new Date(dateStr + 'T23:59:59').getTime() - Date.now()) / 86400000,
    );
}
...
function diffCourses(
    prev: ClassroomCourse[],
    next: ClassroomCourse[],
): DiffResult {
    ...
    for (const nc of next) {
        ...
        const prevGradeIds = new Set(
            pc?.gradedSubmissions.map((g) => g.courseWorkId) ?? [],
        );
...
async function fetchTokenFromDB(): Promise<string | null> {
    try {
        const res = await apiGetClassroomToken();
...
async function fetchAllClassroomData(
    ...
    const { courses: rawCourses = [] } = await gFetch(
        '/v1/courses?courseStates=ACTIVE&pageSize=30',
        token,
    );
    const enriched = await Promise.all(
        rawCourses.map(async (course: any) => {
            const [annRes, cwRes, subRes, tiRes] = await Promise.allSettled([
                gFetch(
                    `/v1/courses/${course.id}/announcements?pageSize=5&orderBy=updateTime%20desc`,
                    token,
                ),
                gFetch(
                    `/v1/courses/${course.id}/courseWork?pageSize=30&orderBy=dueDate%20desc`,
                    token,
                ),
                gFetch(
                    `/v1/courses/${course.id}/courseWork/-/studentSubmissions?states=RETURNED`,
                    token,
                ),
                gFetch(
                    `/v1/courses/${course.id}/courseWork/-/studentSubmissions?states=TURNED_IN`,
                    token,
                ),
            ]);
            const announcements =
            ...
            const gradedSubmissions =
                subRes.status === 'fulfilled'
                    ...
                    : [];
            const tiIds: string[] =
                tiRes.status === 'fulfilled'
                    ? (tiRes.value.studentSubmissions ?? []).map(
                          (s: any) => s.courseWorkId,
                      )
                    : [];
...
function CoursePill({
    ...
    const pendingCount = course.coursework.filter(
        (cw) =>
            cw.dueDate &&
            daysUntil(cw.dueDate) >= 0 &&
            !course.turnedInIds.includes(cw.id),
    ).length;
    return (
        <button
            onClick={onClick}
            ...
            {hasCritical && (
                <span
                    style={{
                        ...
                        background: '#E24B4A',
                        flexShrink: 0,
                        boxShadow: '0 0 0 2px rgba(226,75,74,0.2)',
                    }}
                />
            )}
            <span
...
function AssignmentRow({
    ...
    const urgency =
        days !== null && !isSubmitted && !hasGrade ? urgencyLabel(days) : null;
    const pct =
        grade && cw.maxPoints
            ? Math.round((grade.assignedGrade / cw.maxPoints) * 100)
            : null;
    const gc = gradeColor(pct);
    const metaParts = [
        WORK_TYPE_LABEL[cw.workType] ?? cw.workType,
        cw.maxPoints != null ? `${cw.maxPoints} pts` : null,
    ]
        .filter(Boolean)
        .join(' · ');
...
function GradeRow({
    ...
    const pct = cw?.maxPoints
        ? Math.round((sub.assignedGrade / cw.maxPoints) * 100)
        : null;
    const gc = gradeColor(pct);
    return (
        <div
...
export default function ClassroomPage() {
    ...
    const [nextRefreshAt, setNextRefreshAt] = useState(
        Date.now() + AUTO_REFRESH_MS,
    );
    const [syncingCourse, setSyncingCourse] = useState<ClassroomCourse | null>(
        null,
    );
        ...
        async (silent = false) => {
            ...
            try {
                ...
                if (!token) {
                    setLinked(false);
                    setError(
                        'Session expired. Please re-link Google Classroom.',
                    );
                    return;
                }
    ...
    const connectClassroom = useGoogleLogin({
        ...
        onSuccess: async (codeResponse) => {
            try {
                await sendClassroomAuthCode(codeResponse.code);
                setLinked(true);
    ...
    const filteredWork = useMemo(() => {
        ...
        return allWork.filter((cw) => {
            ...
            const grade = selected.gradedSubmissions.find(
                (s) => s.courseWorkId === cw.id,
            );
            if (filterType === 'pending' && (isSubmitted || !!grade))
                return false;
            if (filterType === 'submitted' && !isSubmitted && !grade)
                return false;
    ...
    const upcomingWork = filteredWork.filter(
        (cw) => daysUntil(cw.dueDate!) >= 0,
    );
    const pastWork = filteredWork.filter((cw) => daysUntil(cw.dueDate!) < 0);

    const totalDueToday = courses.reduce(
        (n, c) =>
            n +
            c.coursework.filter(
                (cw) =>
                    cw.dueDate &&
                    daysUntil(cw.dueDate) === 0 &&
                    !c.turnedInIds.includes(cw.id),
            ).length,
        0,
    );
    const totalDueWeek = courses.reduce(
        (n, c) =>
            n +
            c.coursework.filter(
                (cw) =>
                        ...
                        >
                            ...
                            <div
                                style={{
                                    ...
                                    animation:
                                        'spin 1.4s linear infinite reverse',
                                }}
                            />
                            <div
                                style={{
                    ...
                    >
                        <div style={{ textAlign: 'center', maxWidth: 380 }}>
                            ...
                            >
                                ...
                                deadlines, see grades, and sync everything to
                                your calendar.
                            </p>
                            {error && (
                                <div
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    >
                                        Your Classroom data (courses,
                                        assignments, grades, and announcements)
                                        is strictly confidential. It syncs
                                        directly from Google to your local
                                        device.{' '}
                                        <span
                                            style={{
                                                fontWeight: 700,
                                        ...
                                        >
                                            We do not transmit or store any of
                                            this information.
                                        </span>
                                    </p>
                                </div>
                ...
                {linked && courses.length > 0 && (
                    ...
                    >
                        ...
                        >
                            <StatCard
                                ...
                                sub={
                                    totalDueToday === 0
                                        ? 'all clear'
                                        : `${totalDueToday} assignment${totalDueToday > 1 ? 's' : ''}`
                                }
                                accentColor='#E24B4A'
                                borderColor='rgba(226,75,74,0.4)'
                                active={totalDueToday > 0}
                            />
                            <StatCard
                                label='This week'
                                value={totalDueWeek}
                        ...
                        >
                            ...
                            >
                                {courses.map((course) => {
                                    const hasCritical = course.coursework.some(
                                        (cw) =>
                                            cw.dueDate &&
                                            daysUntil(cw.dueDate) >= 0 &&
                                            daysUntil(cw.dueDate) <= 1 &&
                                            !course.turnedInIds.includes(cw.id),
                                    );
                                    return (
                                        <CoursePill
                                            ...
                                            isSynced={syncedIds.has(course.id)}
                                            onClick={() =>
                        ...
                        {selected && (
                            ...
                            >
                                            ...
                                            [
                                                {
                                                    ...
                                                    label: 'Coursework',
                                                    icon: BookOpen,
                                                    count: selected.coursework.filter(
                                                        (cw) => cw.dueDate,
                                                    ).length,
                                                },
                                                {
                                ...
                                >
                                                ...
                                                <>
                                                    ...
                                                    {upcomingWork.map((cw) => (
                                                        <AssignmentRow
                                                            key={cw.id}
                                                            cw={cw}
                                                            isSubmitted={selected.turnedInIds.includes(
                                                                cw.id,
                                                            )}
                                                            grade={selected.gradedSubmissions.find(
                                                                (s) =>
                                                                    s.courseWorkId ===
                                                                    cw.id,
                                                            )}
                                    ...
                                    {activeTab === 'grades' &&
                                        (selected.gradedSubmissions.length ===
                                        0 ? (
                                            <EmptyState
                                                icon={Star}
