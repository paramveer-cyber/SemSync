...
function loadItems(): LocalItem[] {
    return getConfiguration().calendarItems as LocalItem[];
}
function saveItems(items: LocalItem[]) {
    updateConfiguration({ calendarItems: items });
}

function getMeta(e: any) {
    if (e._local)
        return (
            TYPE_META[e.type === 'task' ? 'task' : (e.evalType ?? 'other')] ??
            TYPE_META.other
        );
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    color: item.done
                                        ? '#a78bfa'
                                        : 'var(--color-brand)',
                                    border: `1px solid ${item.done ? 'rgba(167,139,250,0.3)' : 'var(--color-brand)'}`,
                                }}
                            >
                                <CheckCircle2
...
export default function CalendarPage() {
    ...
    const load = useCallback(async () => {
        try {
            ...
            const details = await Promise.all(
                courseList.map((c: any) => fetchCourse(c.id).catch(() => null)),
            );
            const gathered: any[] = [];
            details.forEach((d, i) => {
                if (!d) return;
    ...
    const evalsOnDay = (day: number) => {
        ...
        const remote = allEvals.filter((e) => {
            ...
            return (
                ed.getFullYear() === target.getFullYear() &&
        ...
        const local = localItems.filter((item) => {
            ...
            return (
                y2 === target.getFullYear() &&
                m2 - 1 === target.getMonth() &&
                d2 === target.getDate()
            );
        });
        return {
            remote,
            local,
            all: [...remote, ...local.map((l) => ({ ...l, _local: true }))],
        };
    };
            ...
            <div className='grow flex flex-col overflow-hidden'>
                ...
                >
                    ...
                    <div className='flex items-center gap-2'>
                        <button
                            onClick={prevMonth}
                            className='w-9 h-9 border border-[var(--color-glass-border)] flex items-center justify-center hover:border-[var(--color-text-muted)] transition-colors rounded-lg'
                        ...
                        <button
                            ...
                            className='px-4 py-2 border border-[var(--color-glass-border)] text-3xs font-bold tracking-widest hover:border-[var(--color-brand)] hover:text-[var(--color-brand)] transition-colors uppercase rounded-lg'
                            style={{ background: 'var(--color-glass)' }}
                        >
                            Today
                        </button>
                        ...
                        {selected && (
                            <button
                                onClick={() =>
                                    setModal({
                                        date: new Date(year, month, selected),
                                    })
                                }
                                className='flex items-center gap-2 px-4 py-2 rounded-lg text-2xs font-black tracking-widest uppercase cursor-pointer transition-all'
                                style={{
                    ...
                    <div className='grow flex flex-col overflow-auto'>
                        <div className='grid grid-cols-7 border-b border-[var(--color-glass-border)] shrink-0 min-w-[640px] md:min-w-0'>
                            {DAYS.map((d, i) => (
                                <div
                                    key={d}
                                    className='border-r last:border-r-0 border-[var(--color-glass-border)] px-3 py-3 text-3xs font-extrabold tracking-widest uppercase'
                                    style={{
                                        background: 'var(--color-surface-2)',
                                        color:
                                            i === 0 || i === 6
                        ...
                        <div className='grid grid-cols-7 grow min-w-[640px] md:min-w-0'>
                            {Array.from({ length: firstDay }).map((_, i) => (
                                <div
                                    key={`empty-${i}`}
                                    className='border-r border-b border-[var(--color-glass-border)] min-h-28'
                                    style={{
                            ...
                            {(['day', 'upcoming'] as const).map((tab) => (
                                <button
                                    ...
                                    style={{
                                        ...
                                        border: 'none',
                                        borderBottom: `2px solid ${sideTab === tab ? 'var(--color-brand)' : 'transparent'}`,
                                        color:
                                            sideTab === tab
                                                ? 'var(--color-brand)'
                                                : 'var(--color-text-faint)',
                                ...
                                >
                                    ...
                                    >
                                        <div>
                                            ...
                                            >
                                                {selected
                                                    ? `${MONTHS[month].slice(0, 3)} ${pad2(selected)}`
                                                    : 'Select a day'}
                                            </p>
                                        </div>
                                        {selected && (
                                    ...
                                    {selectedData.all.length > 0 && (
                                        ...
                                        >
                                            {selectedData.all.length} item
                                            {selectedData.all.length !== 1
                                                ? 's'
                                                : ''}{' '}
                                            · tap to view details
                                        </p>
                                                ...
                                                <button
                                                    ...
                                                    style={{
                                                        ...
                                                        background: isActive
                                                            ? meta.bg
                                                            : 'var(--color-surface-2)',
                                                        border: `1px solid ${isActive ? meta.border : 'var(--color-glass-border)'}`,
                                                        cursor: 'pointer',
                                                ...
                                                style={{
                                                    ...
                                                    color:
                                                        upcomingFilter === f
                                                            ? 'var(--color-brand)'
                                                            : 'var(--color-text-faint)',
                                                    border: `1px solid ${upcomingFilter === f ? 'var(--color-brand)' : 'var(--color-glass-border)'}`,
                                                    transition: 'all 0.12s',
                                                }}
                                            >
                                                {f === 'all'
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    style={{
                                                        ...
                                                        background:
                                                            'var(--color-surface-2)',
                                                        border: `1px solid ${isCritical ? 'rgba(239,68,68,0.25)' : 'var(--color-glass-border)'}`,
                                                        cursor: 'pointer',
                                                        transition: 'all 0.12s',
                                                    }}
                                                    onMouseEnter={(el) => {
