...
const getWeekLabel = () => {
    ...
    const week = Math.ceil(
        ((now.getTime() - start.getTime()) / 86_400_000 + start.getDay() + 1) /
            7,
    );
    const month = now
...
function parseClassroomUpcoming(): {
    id: string;
    title: string;
    courseName: string;
    dueDate: string;
    ...
    try {
        ...
        data.forEach((course: any) => {
            const hue = course.name
                .split('')
                .reduce(
                    (h: number, c: string) => (h * 31 + c.charCodeAt(0)) % 360,
                    0,
                );
            (course.coursework ?? []).forEach((cw: any) => {
                if (!cw.dueDate) return;
                const days = Math.ceil(
                    (new Date(cw.dueDate + 'T23:59:59').getTime() -
                        Date.now()) /
                        86400000,
                );
...
export default function Dashboard() {
    ...
    const classroomUpcoming = useMemo(
        () => (classroomLinked ? parseClassroomUpcoming() : []),
        [classroomLinked],
    );
    ...
    const displayUpcoming = useMemo(
        () => (thisWeek.length > 0 ? thisWeek : upcoming.slice(0, 6)),
        [thisWeek, upcoming],
    );

    const isFallback = thisWeek.length === 0 && upcoming.length > 0;
    const { week, month } = useMemo(getWeekLabel, []);
                    ...
                    <section id='upcoming-banner'>
                        ...
                        >
                            ...
                            <div className='relative z-10 min-w-0 flex-1'>
                                ...
                                >
                                    Gamification layer with XP, achievements,
                                    streaks, rewards, and level progression.
                                </p>
                            </div>
                            <button
                                type='button'
                                ...
                                onMouseLeave={(e) => {
                                    ...
                                    b.style.color = 'var(--color-text-faint)';
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
                                {courses.map((course) => {
                                    ...
                                    const barPct = Math.min(
                                        100,
                                        course.targetGrade > 0
                                            ? (course.currentGrade /
                                                  course.targetGrade) *
                                                  100
                                            : 0,
                                    );
                                    const isActing = acting === course.id;
                                        ...
                                        >
                                            {isActing && (
                                                <div className='absolute inset-0 bg-[var(--color-surface)]/80 flex flex-col items-center justify-center gap-3 z-20'>
                                                    <div
                                                        className='w-5 h-5 border border-current border-t-transparent animate-spin rounded-full'
                                                        style={{
                                                            color: 'var(--color-brand)',
                                                        }}
                                                    />
                                            ...
                                            >
                                                <div
                                                    ...
                                                    style={{
                                                        background:
                                                            'repeating-linear-gradient(0deg,transparent,transparent 2px,var(--color-brand-glow) 2px,var(--color-brand-glow) 4px)',
                                                    }}
                                                />
                                                <div
                                                    className='absolute left-0 top-0 bottom-0 w-0.5 opacity-0 group-hover:opacity-100 transition-all duration-150'
                                                    style={{
                                                        background:
                                                            'linear-gradient(180deg,var(--color-brand),var(--color-brand-glow))',
                                                    }}
                                                />
