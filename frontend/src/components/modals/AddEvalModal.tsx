...
export default function AddEvalModal({ courseId, onClose, onCreated, existingWeight = 0 }: { courseId: string; onClose: () => void; onCreated: (e: any) => void; existingWeight?: number }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(newWeight) || newWeight <= 0) return setError('Weightage must be greater than 0');
    if (r1(newWeight) > remainingWeight) return setError(`Weightage exceeds available weight. You can add at most ${remainingWeight}% more.`);
    const score = form.score !== '' ? parseFloat(form.score) : null;
    const maxScore = parseFloat(form.maxScore);
    if (score !== null && score > maxScore) return setError('Score cannot exceed Max Score');
    setLoading(true);
    try {
    ...
    <Modal title="Add Evaluation" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className={lCls} style={lSty}>Weight % <span style={{ color: remainingWeight <= 10 ? '#ef4444' : 'var(--color-text-faint)', fontWeight: 'normal' }}>({remainingWeight}% left)</span></label>
            <input className={iCls} style={iSty} type="number" placeholder="20" min="0.1" max={remainingWeight} step="0.1" value={form.weightage} onChange={set('weightage')} />
          </div>
          <div>
