...
export default function AddCourseModal({
    ...
    const updateRow = useCallback(
        (id: string, field: keyof StructureRow, value: string) =>
            setRows((prev) =>
    ...
    const goToStep2 = useCallback(() => {
        if (!form.name.trim()) {
            setStep1Error('Course name is required');
            return;
        }
        setStep1Error('');
        setStep(2);
    ...
    const handleSkip = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const { course } = await createCourse(buildCoursePayload());
            onCreated(course);
            onClose();
        } catch (err: any) {
    ...
    }, [buildCoursePayload, onCreated, onClose]);
            ...
            for (const row of rows) {
                ...
                for (let i = 1; i <= count; i++) {
                    const title =
                        count === 1
                            ? row.label.trim() ||
                              EVAL_TYPES.find((t) => t.value === row.type)
                                  ?.label ||
                              row.type
    ...
    }, [weightOk, buildCoursePayload, rows, onCreated, onClose]);
                ...
                >
                    ...
                    <div className='flex items-center gap-4'>
                                ...
                                >
                                    <div
                                        ...
                                        style={{
                                            ...
                                            background:
                                                step === s
                                                    ? 'rgba(var(--color-brand-raw,34,197,94),0.1)'
                                                    : 'transparent',
                                        }}
                                    >
                                        {s}
                        ...
                        <button
                            ...
                            disabled={loading}
                            className='w-8 h-8 flex items-center justify-center cursor-pointer transition-all duration-150 disabled:opacity-40'
                            style={{
                                border: '1px solid var(--color-glass-border)',
                                background: 'var(--color-glass)',
                            ...
                            <button
                                ...
                                onMouseLeave={(e) => {
                                    (
                                        ...
                                        'rgba(var(--color-brand-raw,34,197,94),0.08)';
                                    (
                                        e.currentTarget as HTMLButtonElement
                                    ).style.color = 'var(--color-brand)';
                                    (
                    ...
                    <div className='p-8'>
                        ...
                        >
                            <div>
                                ...
                                <p className='text-base font-extrabold tracking-tight uppercase text-[var(--color-text)]'>
                                    {form.name}
                                </p>
                            </div>
                            <div className='text-right'>
                                ...
                                <p className='text-base font-extrabold font-mono text-[var(--color-text)]'>
                                    {form.targetGrade}%
                                </p>
                            </div>
                        </div>
                        ...
                        <div className='mb-2'>
                            ...
                            <div
                                className='h-2 w-full flex overflow-hidden'
                                style={{
                        ...
                        <div className='flex gap-3'>
                            <button
                                onClick={() => setStep(1)}
                                disabled={loading}
                                className='flex items-center gap-2 px-6 py-3 text-sm font-black tracking-widest uppercase cursor-pointer transition-all duration-150 disabled:opacity-40'
                                style={{
                                    background: 'transparent',
                                    color: 'var(--color-text-muted)',
                                    border: '1px solid var(--color-glass-border)',
