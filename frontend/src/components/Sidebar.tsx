import { LayoutDashboard, BookOpen, CalendarDays, ClipboardList, Settings, LogOut, Timer, GraduationCap, Trophy } from 'lucide-react';
    ...
    >
          ...
          return (
            ...
            >
              <item.icon
                ...
                style={{ color: active ? 'var(--color-active-text)' : 'var(--color-text-faint)' }}
              />
              <span
                className="text-sm font-medium"
                style={{ color: active ? 'var(--color-active-text)' : 'var(--color-text-muted)' }}
              >
                {item.name}
              </span>
              {active && (
                <div className="ml-auto w-1.5 h-1.5 rounded-full" style={{ background: 'var(--color-brand)' }} />
              )}
            </Link>
          );
        })}
      ...
      {xpState && !isProgressPage && (
        <div className="px-4 pt-2 pb-1">
          <XPBar totalXp={xpState.totalXp} level={xpState.level} compact />
          {xpState.currentStreak > 0 && (
            <div className="flex items-center gap-2 px-4 py-1.5">
              <span className="text-base">{xpState.currentStreak >= 30 ? '⭐' : '🔥'}</span>
              <span className="text-xs font-black font-mono" style={{ color: 'var(--color-brand)' }}>
